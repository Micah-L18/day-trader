...
import type { ConnectionState, ConnectionStatus, ProviderKind } from '@shared/types'
...
export class ProviderManager {
  ...
  private wireAndStart(kind: ProviderKind): void {
    ...
    this.unwire = [
      md.on('quote', (q) => this.broadcast('stream:quote', q)),
      ...
      bk.on('positions', (p) => this.broadcast('stream:positions', p)),
      bk.on('account', (a) => this.broadcast('stream:account', a))
    ]
    md.start()
    bk.start()
